...
void worstfit(int p, int *process, int b, int *block) {
    ...
    for (int i = 0; i < p; i++) {
        int worstBlock = -1; // Index of the best fit block
        int maxBlockSize = INT_MIN; // Minimum size of the best fit block

        // Iterate through each block to find the best fit for the current process
        for (int j = 0; j < b; j++) {
            if (block[j] >= process[i] && blockallocated[j] == 0) {
                if (block[j] > maxBlockSize) {
                    maxBlockSize = block[j];
                    worstBlock = j;
                }
            }
        }
